...
struct SMData {
    ...
    std::size_t m_ttot{};
    double m_d{};
    std::array<std::size_t, num_children> children{};
    std::size_t depth{};
...
class AmortSM {
    ...
    auto get_prob_path(IdxContext const &ctx) {
        ...
        cpps.emplace_back(IdxAndProb{.idx=0, .prob=m_vec[0].get_prior()});
        for (IdxContext c = ctx; c; c.pop()) {
            ...
            if (!m_vec[parent_idx].children[c.back()]) {
            ...
            // Push
            cpps.emplace_back(IdxAndProb{.idx=child_idx,
...
int main() {
    ...
    entropy_test([]() {
        return SequenceMemoizerAmort<ByteAlphabet>(12);
    });

    // correctness_and_entropy_test([]() {
    //     return SequenceMemoizerAmort<ByteAlphabet>(8);
    // });
